...
import {getLanguagesList} from "../../store/language/language.action";
import {
    selectLanguagesPending,
    selectLanguagesList
} from "../../store/language/language.selector";
                            ...
                            <div className="col-sm-6 col-lg-4">
                                ...
                                <select name="" id="" className="form-control">
                                    {languages && languages.map(lang => {
                                        return <option key={lang.id}>{lang.title}</option>
                                    })}
                                </select>
                                </div>
                            </div>
...
const mapStateToProps = state => {
    return {
        languages : selectLanguagesList(state),
        isLoading : selectLanguagesPending(state)
    }
};

const mapDispatchToProps = dispatch => {
    return {
        getLanguagesList: data => dispatch(getLanguagesList(data)),
    };
};
